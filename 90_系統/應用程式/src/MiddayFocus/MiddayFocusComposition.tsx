  ...
  return (
    <AbsoluteFill style={{ backgroundColor: 'black' }}>
      ...
      <Sequence from={introFrames} durationInFrames={heygenFrames}>
        ...
        {/* 動態小影片：獨立畫面，會蓋掉講者與截圖，但字幕與招牌留在它上面。
            mp4 本身就是 1080×1920 全畫面（背景與掃光在影片裡已鋪滿），內容只畫在
            y310–1440（safeTop 沿用上面截圖黃框那個量過的值），所以這裡滿版貼上即可，
            不必補底色或算位置。沒標動態時 MOTION_RUNS 是空陣列＝完全沒有這一層。
            ⚠️ 這一段在 `<Sequence from={introFrames}>` 內，時間自動對齊主段（開場卡不算）。 */}
        <MotionOverlay
          runs={MOTION_RUNS}
