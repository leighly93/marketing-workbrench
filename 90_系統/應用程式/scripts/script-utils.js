...
//   - 數字用的半形 , . % 與斜線 / ／（44,396、1.95%、9/16）：都不列入下面任何一組 → 會被「保留成一般字元」，
//     既會顯示在字幕上（字幕文字是從對齊到的腳本字元重建的），又因為不在 BREAK_RE 而不會斷行。
//   - OTHER_PUNCT_RE（略過、不顯示、不斷句）：引號/括號/半形 ! ? : ; 等，維持濾掉。
...
// ⚠️ 2026-09-15 使用者：「把 / 保留在字幕上，目標是 9/16 完整顯示」。在這之前斜線跟引號括號
//    一起被濾掉 —— 08-10 那次只把 , . % 救出來，斜線是當時刻意留著濾的（原註解寫「與 ／/ 等，
//    維持濾掉」），所以 9/16 一直渲染成 916。
// ⚠️ 副作用：斜線進 cleaned chars 之後，它後面每個字的 charIdx 都會 +1。新工作沒影響
//    （整條產線同一次算出來），但舊工作已經存好的標注／計畫編輯拿來重跑會差一格。
// ⚠️ 「不斷句」跟「不折行」是兩層，這裡只管不斷句：CSS 把 / 當成可折行機會，
//    要避免「9/」「16」拆成兩行是 src/Subtitles.tsx 的 keepTokensTogether（跟減號同一套）。
const BREAK_RE = /[，。、！？：；\n]/;
