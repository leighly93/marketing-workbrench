#!/usr/bin/env node
/**
 * 盤中焦點專用腳本解析：解析 public/script.txt 內的 (shot:名稱)...(shot:名稱) 標記，
 * 產生 src/MiddayFocus/midday-shots.generated.json，並把當天日期（MMDD）寫進
 * src/video-meta.json 的 headerDate 欄位。
 *
 * 2026-08-31 新增，內容照 parse-dapan-script.js 複製一份（天條 #4 精神延伸，不抽共用層）：
 ...
 * 三支都會讀寫 src/video-meta.json，但只碰自己的欄位（titleText / headerDate），互不覆蓋。
 *
 * 只解析 (shot:名稱)，不含 (imageN)/(logo)/(text:)（跟大盤小報同範圍）。
 * 黃框由 npm run auto-shot:midday ＋ 前台人工標注決定，不在這支處理。
 *
 * 用法：npm run parse-script:midday
 */
...
const OUTPUT_DIR = path.join(ROOT, 'src', 'MiddayFocus');
const OUTPUT_PATH = path.join(OUTPUT_DIR, 'midday-shots.generated.json');
...
// 因為盤中焦點使用者的 script.txt 目前多打了一個空的 === 區塊（4 段、不是標準 3 段），
