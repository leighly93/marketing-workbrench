...
- `ocrDigits(image)`：數字編號定位用，回傳至少含 `t/y/h` 的字框；Tesseract 保留 psm 10 與 1234 白名單，Vision 走整頁辨識。（原本的呼叫端是三大法人，該版型 2026-09-22 移除，介面本身保留。）
...
- `safeTop` 直接沿用各 composition 自己那個量過的值（招牌實心到 y 幾：盤中焦點 291／大盤小報 303／美股焦點 307），動態要避開的東西跟截圖黃框完全一樣，沒有理由另立一套。`safeBottom` 全部相同 —— 直式版型共用 `Subtitles.tsx`，字幕一律從 y1440 起。
...
- 現存三個版型都已接上動態小影片。
- **動態與配圖重疊不擋**（2026-09-21 使用者定案：「蓋掉就算了」）。同一段既標了配圖又標了動態時，動態會蓋掉截圖 —— `MotionOverlay` 疊在截圖層之後。這是刻意允許的，不要再加檢查或警告。
